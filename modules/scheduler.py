...
logger = logging.getLogger('rtm.scheduler')
...
class Scheduler:
    ...
    def make_tasks(self):
        ...
        for rule_file in os.listdir(rule_path):
            with open(f'{rule_path}/{rule_file}', encoding="utf-8") as f:
                rule = yaml.load(f.read(), Loader=yaml.FullLoader)
            if not rule.get("enable", True):
                logger.info(f"Rule [{rule.get('rule_name', None)}] is disable. Not scheduled.")
                continue
            if rule:
                self.add_job(rule)
